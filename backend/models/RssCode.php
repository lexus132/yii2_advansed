	...
	public static function parseCode($url) {
		...
		if($loadFlag === true) {
			...
			foreach($items as $codeOneItem) {
		        ...
		        //Получаем img
		        $newsLinkObject = $codeOneItem->getElementsByTagName('enclosure');
		        $img = $newsLinkObject->item(0)->getAttribute('url');
		        ...
		        $rssData[] = ['title' => $title,'img' => $img, 'description' => $description, 'link' => $link, 'publicationDate' => $publicationDate];
	    	}
